$(document).ready(function() {
  ...
  $('[aria-controls]').on('click', function() {
    ...
    $('[aria-controls]').not(this).each(function() {
      ...
      if (!button.parents('li').has(otherButton).length) {
        ...
        otherButton.find('svg').removeClass('rotate-90 text-gray-500').addClass('text-gray-400');
        otherTargetUl.slideUp().addClass('collapsed-menu');
      }
    });

    // Collapse sibling elements
    button.closest('li').siblings().each(function() {
      ...
      siblingButton.find('svg').removeClass('rotate-90 text-gray-500').addClass('text-gray-400');
    ...
    // Toggle chevron icon
    if (!isExpanded) {
      button.find('svg').addClass('rotate-90 text-gray-500').removeClass('text-gray-400');
      targetUl.removeClass('collapsed-menu').removeAttr('style');
    } else {
      button.find('svg').removeClass('rotate-90 text-gray-500').addClass('text-gray-400');
